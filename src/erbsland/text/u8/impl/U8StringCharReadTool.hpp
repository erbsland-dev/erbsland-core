...
public:
    explicit constexpr U8StringCharReadTool(const U8StringDataView &data) noexcept : _data{data} {}
...
public: // read
    ...
    /// Get the start byte index for the given character index.
    [[nodiscard]] auto byteIndexAt(unit::CpIndex index) const noexcept -> unit::ByteIndex;
    /// Get the character index for the given byte index.
    [[nodiscard]] auto charIndexAt(unit::ByteIndex index) const noexcept -> unit::CpIndex;
    /// Convert a relative character range for this view into a bounded byte range for the backing data.
    [[nodiscard]] auto sliceRange(unit::CpRange range) const noexcept -> unit::ByteRange;
...
public: // find
    ...
    [[nodiscard]] auto find(const U8StringDataView &text) const noexcept -> unit::CpIndex;
    /// Find the first decoded character sequence at or after the given character index.
    [[nodiscard]] auto find(const U8StringDataView &text, unit::CpIndex start) const noexcept -> unit::CpIndex;
...
private:
    ...
    [[nodiscard]] static auto matchesCharacterSequence(
        std::span<const char> haystack, unit::ByteIndex candidateStart, std::span<const char> needle) noexcept -> bool;
