...
public:
    ...
    void testReadableBufferToMaskOverloadsDelegateToToMaskImpl() {
        ...
        REQUIRE_EQUAL(buffer.toMask(erbsland::text::CharSet{"AB"_el}).size(), (block::Size{2, 1}));
        REQUIRE_EQUAL(buffer._characters, erbsland::text::CharSet{"AB"_el});
        REQUIRE_FALSE(buffer._invert);

        REQUIRE_EQUAL(buffer.toMask({U'X', U'Y'}, true).size(), (block::Size{2, 1}));
    ...
    void testWritableBufferSetFromAndFillDelegateToImplMethods() {
        ...
        buffer.fill(block::Rectangle{0, 0, 2, 2}, Block{U'.'});
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::FillBlock);
        REQUIRE_EQUAL(buffer._lastRect, (block::Rectangle{0, 0, 2, 2}));
        REQUIRE_EQUAL(buffer._lastFillChar, Block{U'.'});

        buffer.clearRecording();
        buffer.fill(block::Rectangle{0, 0, 3, 3}, Tile9Style::create("ABCDEFGHI"_el), Color{fg::Blue, bg::Black});
    ...
    void testWritableBufferResizeWithModeUsesTheDefaultImplementation() {
        ...
        buffer.resize(block::Size{5, 3}, BufferResizeMode::Fast, Block{U'.'});
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::Resize);
        REQUIRE_EQUAL(buffer._lastResizeSize, (block::Size{5, 3}));
        REQUIRE_EQUAL(buffer._resizeCallCount, 1);

        buffer.clearRecording();
        buffer.resize(block::Size{6, 4}, BufferResizeMode::PreserveContent, Block{U'.'});
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::SetFrom);
        REQUIRE_EQUAL(buffer._lastFillChar, Block{U'.'});
        REQUIRE_EQUAL(buffer._resizeCallCount, 1);
        REQUIRE_EQUAL(buffer._lastResizeSize, (block::Size{6, 4}));
    }

    void testWritableBufferFrameWrappersDelegateToGroupedImplMethods() {
        auto buffer = WritableBufferDispatchProbe{};

        buffer.drawFrame(block::Rectangle{0, 0, 2, 2}, Block{U'#'});
        ...
        buffer.clearRecording();
        buffer.drawFilledFrame(block::Rectangle{0, 0, 2, 2}, Block{U'#'}, Block{U'.'});
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::FrameBlock);
        REQUIRE(buffer._lastOptionalFillBlock.has_value());
        REQUIRE_EQUAL(*buffer._lastOptionalFillBlock, Block{U'.'});

        buffer.clearRecording();
        buffer.drawFrame(block::Rectangle{0, 0, 3, 3}, FrameStyle::Light, Color{fg::Green, bg::Black});
        ...
        buffer.clearRecording();
        buffer.drawFilledFrame(block::Rectangle{0, 0, 3, 3}, FrameStyle::OuterHalfBlock, Block{U'.'});
        ...
        buffer.clearRecording();
        buffer.drawFrame(block::Rectangle{0, 0, 3, 3}, FrameDrawOptions::defaultOptions(), 7);
        ...
        buffer.drawGridLayout(block::Position{1, 2}, gridLayout({2, 3}, {1, 2}), border);
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::GridLayout);
        REQUIRE_EQUAL(buffer._lastPosition, (block::Position{1, 2}));
        REQUIRE(buffer._lastGridLayout.has_value());
        REQUIRE_EQUAL(buffer._lastGridLayout->size(border), (block::Size{8, 6}));
    ...
    void testWritableBufferTextWrappersDelegateToImplMethods() {
        ...
        buffer.drawBlockText(BlockText{BlockStringEditor{"Hello"_el}, block::Rectangle{1, 2, 3, 4}});
        REQUIRE_EQUAL(buffer._lastCall, WritableBufferDispatchProbe::Call::TextObject);
        REQUIRE_EQUAL(buffer._lastRect, (block::Rectangle{1, 2, 3, 4}));
    ...
    void testWritableBufferBitmapWrappersDelegateToImplMethods() {
        ...
        REQUIRE_EQUAL(buffer._lastBitmapSize, (block::Size{2, 3}));
        REQUIRE_EQUAL(buffer._lastRect, (block::Rectangle{0, 0, 4, 5}));
        REQUIRE_EQUAL(buffer._lastAlignment, geometry::Alignment::BottomRight);
