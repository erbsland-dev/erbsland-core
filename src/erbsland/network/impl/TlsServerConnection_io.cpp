...
void TlsServerConnection::handleTransportConnected() {
    ...
    if (!canContinue() || _state.load() != TlsServerConnectionState::Accepting || _protocol == nullptr) {
        return;
    }
    _state.store(TlsServerConnectionState::Handshaking);
    try {
...
void TlsServerConnection::handleTransportData(mem::ByteBlock data) {
    ...
    if (!canContinue() || _protocol == nullptr ||
        (current != TlsServerConnectionState::Handshaking && current != TlsServerConnectionState::Active &&
            current != TlsServerConnectionState::Closing)) {
        return;
    }
...
void TlsServerConnection::handleTransportWritable() {
    ...
    pumpTransportOutput();
    assessProtocolState();
    if (_applicationBlocked && !_pendingTransportRecord.has_value() &&
        _state.load() == TlsServerConnectionState::Active) {
...
void TlsServerConnection::serviceCheckpoints() {
    while (canContinue() && _protocol != nullptr && _protocol->hasCheckpoint()) {
        ...
        case TlsServerProtocolCheckpoint::HandshakeCompleted:
            _state.store(TlsServerConnectionState::Active);
        ...
        if (status == NetworkSendStatus::Closed) {
            finishFailed(
                NetworkErrorContext{"TLS transport write failed"_el, "TCP rejected a complete TLS record."_el}
                    ...
                    .setPhase(
                        _state.load() == TlsServerConnectionState::Closing ? NetworkErrorPhase::Closing
...
void TlsServerConnection::drainApplicationInput() {
    if (_receivingPaused || _protocol == nullptr || _state.load() != TlsServerConnectionState::Active) {
        return;
    }
...
void TlsServerConnection::assessProtocolState() {
    ...
    if (_protocol->peerCloseNotifyReceived() && !_closeOrigin.has_value()) {
        enterClosing(TlsServerConnectionCloseOrigin::Remote);
    }
