...
auto TcpConnection::state() const noexcept -> NetworkSourceState {
    return _state.load();
}
...
void TcpConnection::deliverError(const std::uint64_t generation, NetworkErrorContext context) {
    ...
    if (_state.load() == NetworkSourceState::Starting && _nextEndpoint < _resolvedEndpoints.count().toSizeT()) {
        _lastConnectError = std::move(context);
        startNextEndpoint(generation);
        return;
    }
    if (_state.load() == NetworkSourceState::Starting && !_resolvedEndpoints.isEmpty()) {
        _lastConnectError = std::move(context);
        startNextEndpoint(generation);
        return;
    }
