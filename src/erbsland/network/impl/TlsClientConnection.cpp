...
auto TlsClientConnection::negotiatedAlpn() const -> mem::ByteBlock {
    const auto lock = std::scoped_lock{_mutex};
...
auto TlsClientConnection::state() const noexcept -> TlsClientConnectionState {
    return _state.load();
}
...
void TlsClientConnection::connect(HostEndpoint endpoint, TlsClientConnectOptions options) {
    ...
    _started.store(true);
    _state.store(TlsClientConnectionState::Connecting);
...
auto TlsClientConnection::send(const mem::ByteBlock &data) -> NetworkSendStatus {
    const auto lock = std::scoped_lock{_mutex};
    verifyCurrentOwnerEvents();
    if (_state.load() != TlsClientConnectionState::Active || _protocol == nullptr) {
...
void TlsClientConnection::pauseReceiving() {
    const auto lock = std::scoped_lock{_mutex};
    verifyCurrentOwnerEvents();
    if (_state.load() != TlsClientConnectionState::Active || _receivingPaused) {
        return;
    }
...
void TlsClientConnection::close() {
    const auto lock = std::scoped_lock{_mutex};
    verifyCurrentOwnerEvents();
    if (_state.load() == TlsClientConnectionState::Inactive || _state.load() == TlsClientConnectionState::Connecting ||
        _state.load() == TlsClientConnectionState::Handshaking) {
        abort();
        return;
    }
    if (_state.load() != TlsClientConnectionState::Active || _protocol == nullptr) {
    ...
    enterClosing(TlsClientConnectionCloseOrigin::Local);
    _protocol->close();
    pumpTransportOutput();
    assessProtocolState();
}
...
void TlsClientConnection::abort() noexcept {
    const auto lock = std::scoped_lock{_mutex};
    const auto previous = _state.exchange(TlsClientConnectionState::Closed);
    if (previous == TlsClientConnectionState::Closed || previous == TlsClientConnectionState::Failed) {
        return;
    }
    _started.store(true);
    ...
    if (previous == TlsClientConnectionState::Inactive) {
        postAbortFinal();
    }
}
...
auto TlsClientConnection::canContinue() const noexcept -> bool {
    ...
    return !_abortRequested.load() && current != TlsClientConnectionState::Closed &&
        current != TlsClientConnectionState::Failed;
}
