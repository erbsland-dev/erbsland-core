...
UnsafeU16StringBuffer::UnsafeU16StringBuffer(unit::U16DataLength capacity) :
...
auto UnsafeU16StringBuffer::capacity() const noexcept -> unit::U16DataLength {
    const auto size = dataSize();
    if (size == 0U) {
        return unit::U16DataLength::zero();
    }
    return unit::U16DataLength::fromSizeT(size - 1U);
}

auto UnsafeU16StringBuffer::take(const unit::U16DataLength length) -> U16String {
    ...
    return U16String{U16StringSharedStorage{std::move(data), unit::U16DataRange::fromSizeT(size)}};
}

auto UnsafeU16StringBuffer::take(const std::size_t length) -> U16String {
    const auto finalLength = unit::U16DataLength::fromSizeT(length);
    return take(finalLength);
}

auto UnsafeU16StringBuffer::takeAsUtf8(const unit::U16DataLength length) -> U8String {
    const auto u16String = take(length);
    return StringConverter{u16String}.toU8String();
}

auto UnsafeU16StringBuffer::takeAsUtf8(std::size_t length) -> U8String {
    return takeAsUtf8(unit::U16DataLength::fromSizeT(length));
}
...
auto UnsafeU16StringBuffer::dataSizeForCapacity(unit::U16DataLength capacity) -> std::size_t {
...
auto UnsafeU16StringBuffer::checkedFinalLength(unit::U16DataLength length) const -> unit::U16DataLength {
