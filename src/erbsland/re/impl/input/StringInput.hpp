...
/// An input that reads from an owning UTF-8 string view.
class StringInput final : public Input {
public:
    [[nodiscard]] static auto create(const text::StringView &text) noexcept -> InputPtr {
        return std::make_shared<StringInput>(text);
    }
    explicit StringInput(const text::StringView &text) noexcept : _text{text} {}
