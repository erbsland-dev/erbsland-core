...
void IoService::submit(std::function<void()> work) {
    {
...
auto IoService::service() -> IoService & {
    static auto *instance = new IoService{}; // Intentionally process-lifetime; never joins blocked native calls.
    return *instance;
}

void IoService::submitIoWork(std::function<void()> work) {
    service().submit(std::move(work));
}
