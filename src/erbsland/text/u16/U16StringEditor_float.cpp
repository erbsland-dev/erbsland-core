...
auto U16StringEditor::fromFloat(const double value, const FloatFormat format) -> U16StringEditor {
    const auto text = impl::formatFloat(value, format);
    return U16StringEditor{StringConverter{text}.toU16String()};
}

template <impl::AnyFloatType T>
auto U16StringEditor::toFloat(T defaultValue, FloatParseOptions options) const noexcept -> T {
...
template <impl::AnyFloatType T>
auto U16StringEditor::toFloatOrThrow(FloatParseOptions options) const -> T {
    if constexpr (std::same_as<T, float>) {
...
template auto U16StringEditor::toFloat<float>(float defaultValue, FloatParseOptions options) const noexcept -> float;
template auto U16StringEditor::toFloat<double>(double defaultValue, FloatParseOptions options) const noexcept -> double;
template auto U16StringEditor::toFloatOrThrow<float>(FloatParseOptions options) const -> float;
template auto U16StringEditor::toFloatOrThrow<double>(FloatParseOptions options) const -> double;
