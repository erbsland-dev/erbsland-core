...
public:
    ...
    explicit RandomError(text::String reason) noexcept : err::LogicError{std::move(reason)} {}
    /// Create a random error with a reason and diagnostic cause.
    /// @param reason The reason for the random error.
    /// @param cause The diagnostic cause.
    explicit RandomError(text::String reason, std::exception_ptr cause) noexcept :
