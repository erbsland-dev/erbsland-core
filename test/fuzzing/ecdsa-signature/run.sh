...
corpusDir="${buildDir}/corpus/ecdsa-signature"
...
cmake --build "${buildDir}" --target ecdsa-signature-fuzz

mkdir -p "${corpusDir}"
for seed in "${scriptDir}"/corpus/*.hex; do
    xxd -r -p "${seed}" "${corpusDir}/$(basename "${seed}" .hex)"
done
exec "${buildDir}/test/fuzzing/ecdsa-signature/ecdsa-signature-fuzz" "${corpusDir}" "$@"
