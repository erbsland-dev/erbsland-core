...
void TlsClientConnection::ensureIdleTimer() {
    if (_idleTimerPending || _state.load() != TlsClientConnectionState::Active) {
        return;
    }
...
void TlsClientConnection::handleHandshakeTimeout(const std::uint64_t generation, const time::TimePoint deadline) {
    ...
    if (generation != _handshakeTimerGeneration || _state.load() != TlsClientConnectionState::Handshaking ||
...
void TlsClientConnection::handleIdleTimeout(const std::uint64_t generation) {
    ...
    if (_state.load() != TlsClientConnectionState::Active) {
        return;
    }
...
void TlsClientConnection::handleCloseTimeout(const std::uint64_t generation, const time::TimePoint deadline) {
    const auto lock = std::scoped_lock{_mutex};
    if (generation != _closeTimerGeneration || _state.load() != TlsClientConnectionState::Closing ||
        time::TimePoint::now() < deadline) {
        return;
    }
    finishFailed(
...
void TlsClientConnection::enterClosing(const TlsClientConnectionCloseOrigin origin) {
    if (_closeOrigin.has_value()) {
        return;
    }
    _closeOrigin = origin;
    _state.store(TlsClientConnectionState::Closing);
...
auto TlsClientConnection::protocolErrorContext() const -> NetworkErrorContext {
    ...
    const auto phase = _state.load() == TlsClientConnectionState::Closing ? NetworkErrorPhase::Closing
        : _state.load() == TlsClientConnectionState::Active               ? NetworkErrorPhase::Active
...
auto TlsClientConnection::enrichTransportError(NetworkErrorContext context) const -> NetworkErrorContext {
    ...
    switch (_state.load()) {
    case TlsClientConnectionState::Inactive:
        phase = NetworkErrorPhase::Configuration;
        break;
    case TlsClientConnectionState::Connecting:
        phase = _hostResolved ? NetworkErrorPhase::Connecting : NetworkErrorPhase::Resolving;
        break;
    case TlsClientConnectionState::Handshaking:
        phase = NetworkErrorPhase::Handshaking;
        break;
    case TlsClientConnectionState::Active:
        phase = NetworkErrorPhase::Active;
        break;
    case TlsClientConnectionState::Closing:
    case TlsClientConnectionState::Closed:
        phase = NetworkErrorPhase::Closing;
        break;
    case TlsClientConnectionState::Failed:
        phase = context.phase();
        break;
    }
    context.setPhase(phase);
...
void TlsClientConnection::finishFailed(NetworkErrorContext context) {
    if (_finalized.load() || _state.load() == TlsClientConnectionState::Failed) {
        return;
    }
    _state.store(TlsClientConnectionState::Failed);
...
void TlsClientConnection::finishClosed() {
    ...
    _state.store(TlsClientConnectionState::Closed);
    ++_closeTimerGeneration;
    const auto context = TlsClientConnectionCloseContext{_closeOrigin.value_or(TlsClientConnectionCloseOrigin::Remote)};
    try {
