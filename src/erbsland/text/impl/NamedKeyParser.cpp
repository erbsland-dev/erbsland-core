...
NamedKeyParser::NamedKeyParser(StringCharReader &reader, const NamedKeyFormat &format) :
    _reader{reader}, _format{format} {
    _format.validate();
}

void NamedKeyParser::setAllowedKeys(util::Set<int> keys) {
    _allowedKeys = std::move(keys);
}

auto NamedKeyParser::readEntry() -> NamedKeyEntry {
    if (_ended) {
        return {};
    }
    if (!_firstEntry) {
...
auto NamedKeyParser::readAllEntries() -> util::List<NamedKeyEntry> {
    auto result = util::List<NamedKeyEntry>{};
    while (true) {
...
auto NamedKeyParser::isAtTerminator() const noexcept -> bool {
...
auto NamedKeyParser::readEnd() -> NamedKeyEntry {
    if (!_format.stopCharacter().isEndOfData()) {
        if (_reader.isAtEnd()) {
...
auto NamedKeyParser::captureRawEntry() -> String {
...
auto NamedKeyParser::parseRawEntry(const String &raw, const unit::CpIndex position) -> NamedKeyEntry {
...
auto NamedKeyParser::parseKeyEntry(
    StringCharReader &reader, const Char prefix, const int keyIndex, const unit::CpIndex position) -> NamedKeyEntry {
    if (reader.isAtEnd()) {
        if (!_format.keysWithoutValuesAllowed()) {
            throwError("Named-key entry is missing a value", position);
        }
        return {NamedKeyEntryKind::Key, prefix, keyIndex, {}};
    }
    ...
    return {NamedKeyEntryKind::KeyWithValue, prefix, keyIndex, std::move(value)};
}

auto NamedKeyParser::parsePositionalValue(const String &raw, const unit::CpIndex position) -> NamedKeyEntry {
    _mode = Mode::Values;
    ++_valueCount;
    ...
    return {NamedKeyEntryKind::Value, Char::noCodePoint(), -1, raw};
}

void NamedKeyParser::validateValue(const String &value, const unit::CpIndex position) const {
...
void NamedKeyParser::markKeySeen(const int keyIndex, const unit::CpIndex position) {
...
void NamedKeyParser::throwError(const std::string_view reason, const unit::CpIndex position) {
