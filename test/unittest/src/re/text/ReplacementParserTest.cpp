...
public:
    ...
    void testParse_EscapedBraces() {
        const CaptureGroupNames groupNames;
        auto parser = ReplacementParser{StringView{"{{}}"_el}, groupNames};
        const auto replacement = parser.parse();

        String out;
        REQUIRE_NOTHROW(replacement.appendTo(out, nullptr));
        REQUIRE_EQUAL(out, String{"{}"_el});
    }

    void testParse_GroupReferencesByIndexByNameAndEmpty() {
        constexpr auto text = "abcd"_el;
        CaptureGroupList groups;
        groups.emplace_back(0, CaptureRange{0, 4}, StringView{});
        ...
        auto parser = ReplacementParser{StringView{"{a}-{b}-{0}-{2}-{1}-{}-{{-}}"_el}, groupNames};
        const auto replacement = parser.parse();

        String out;
        replacement.appendTo(out, match);
        REQUIRE_EQUAL(out, String{"ab-cd-abcd-cd-ab-abcd-{-}"_el});
    }

    void testParse_MixedAndAdjacentExpressions() {
        constexpr auto text = "xy"_el;
        CaptureGroupList groups;
        groups.emplace_back(0, CaptureRange{0, 2}, StringView{});
        ...
        auto parser = ReplacementParser{StringView{"{1}{2}{x}{y}{X}{Y}"_el}, groupNames};
        const auto replacement = parser.parse();

        String out;
        replacement.appendTo(out, match);
        REQUIRE_EQUAL(out, String{"xyxyxy"_el});
    }
    ...
    void testParse_Error_InvalidCharacterAfterGroupIndex() {
        const CaptureGroupNames groupNames;
        auto parser = ReplacementParser{StringView{"{1a}"_el}, groupNames};
    ...
    void testParse_Error_InvalidCharacterAfterGroupName() {
        const CaptureGroupNames groupNames;
        auto parser = ReplacementParser{StringView{"{ab-}"_el}, groupNames};
    ...
    void testParse_Error_UnknownGroupName() {
        ...
        auto parser = ReplacementParser{StringView{"{b}"_el}, groupNames};
    ...
    void testParse_Error_MaxReplacementTextLengthExceeded() {
        ...
        const CaptureGroupNames groupNames;
        auto parser = ReplacementParser{StringView{text}, groupNames};
