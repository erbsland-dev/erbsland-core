...
void HtmlViewerApp::beforeInitialize() {
    ...
    _updateSettings.setMinimumSize(Size{Coordinate{60}, Coordinate{12}});
...
auto HtmlViewerApp::canvasSize() noexcept -> Size {
...
auto HtmlViewerApp::printRenderedDocument() -> int {
    renderDocumentIfRequired(std::max(terminal()->size().width(), Coordinate{1}));
...
void HtmlViewerApp::onKey(const Key &key) {
    ...
    const auto pageStep = std::max(contentRect.height() - Coordinate{1}, Coordinate{1});
    const auto contentHeight = _documentBuffer->size().height();
    if (key == U'q' || key == Key::Escape) {
        _quitRequested = true;
    } else if (key == Key::Up) {
        _viewOffsetY = clampViewOffset(_viewOffsetY - Coordinate{1}, contentRect.height(), contentHeight);
...
void HtmlViewerApp::onRenderToBuffer() {
    ...
    const auto headerRect = Rectangle{Coordinate{0}, Coordinate{0}, _buffer.size().width(), Coordinate{1}};
    const auto contentRect = contentRectForBuffer(_buffer.size());
    const auto footerRect =
        Rectangle{Coordinate{0}, _buffer.size().height() - Coordinate{1}, _buffer.size().width(), Coordinate{1}};
...
void HtmlViewerApp::renderDocumentIfRequired(const Coordinate contentWidth) {
    const auto normalizedWidth = std::max(contentWidth, Coordinate{1});
    ...
    _documentBuffer->resize(Size{normalizedWidth, Coordinate{1}});
...
void HtmlViewerApp::drawFooter(const Rectangle rect) {
    ...
    help.append(
        ...
        fg::BrightWhite,
        " quit"_el);
    _buffer.drawBlockText(BlockText{help, rect.insetBy(Margins{1, 0}), Alignment::CenterRight});
}
...
void HtmlViewerApp::updateView(const Size viewSize) noexcept {
    _viewOffsetY = clampViewOffset(
        _viewOffsetY, viewSize.height(), _documentBuffer == nullptr ? Coordinate{0} : _documentBuffer->size().height());
    _documentView.setViewRect(
        Rectangle{Coordinate{0}, _viewOffsetY, viewSize.width(), std::max(viewSize.height(), Coordinate{1})});
}
...
auto HtmlViewerApp::locationText() const -> el::String {
    const auto totalLines = std::max(_documentBuffer->size().height(), Coordinate{1});
    const auto topLine = std::min(_viewOffsetY + Coordinate{1}, totalLines);
...
auto HtmlViewerApp::contentRectForBuffer(const Size bufferSize) noexcept -> Rectangle {
    return Rectangle{
        Coordinate{0}, Coordinate{1}, bufferSize.width(), std::max(bufferSize.height() - Coordinate{2}, Coordinate{1})};
}

auto HtmlViewerApp::clampViewOffset(
    const Coordinate offset, const Coordinate viewHeight, const Coordinate height) noexcept -> Coordinate {
    return std::clamp(offset, Coordinate{0}, std::max(height - std::max(viewHeight, Coordinate{1}), Coordinate{0}));
}
