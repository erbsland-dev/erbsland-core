...
/// Bounded incremental HTTP/1.x request encoder.
/// @tested{Http1CodecTest}
class Http1RequestEncoder final {
public:
    /// Begin encoding a request.
    explicit Http1RequestEncoder(HttpRequestHead head, Http1CodecLimits limits = {}) :
        _encoder{std::move(head), limits} {}
...
public:
    ...
    /// Finish the request with optional extension trailers.
