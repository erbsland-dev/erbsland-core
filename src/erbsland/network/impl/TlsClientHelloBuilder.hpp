...
public:
    ...
    /// @param alpnProtocols Opaque non-empty ALPN identifiers in preference order.
    /// @throws err::ParameterError If ALPN bounds are exceeded.
    TlsClientHelloBuilder(Host host, std::vector<mem::ByteBlock> alpnProtocols);
...
private:
    ...
    std::vector<mem::ByteBlock> _alpnProtocols; ///< Validated opaque ALPN identifiers.
};
