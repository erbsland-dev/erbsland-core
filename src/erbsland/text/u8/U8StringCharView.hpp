...
/// A specialized owning UTF-8 read-only string with copy-on-write semantics for code-point index access.
...
/// For most use cases, use `StringView`.
/// @tested{U8StringCharViewTest}
class U8StringCharView final {
    friend class U8String;
    friend class U8StringView;
    friend class impl::StringConversionTools;

private:
    explicit U8StringCharView(impl::U8StringViewStorage storage) noexcept : _storage{std::move(storage)} {}

public:
    // default
    ~U8StringCharView() = default;
    U8StringCharView(const U8StringCharView &) = delete;
    U8StringCharView(U8StringCharView &&) = delete;
    auto operator=(const U8StringCharView &) -> U8StringCharView & = delete;
    auto operator=(U8StringCharView &&) -> U8StringCharView & = delete;
...
public: // read
    ...
    /// @seeref{u8-string-char-view-character-based-reading}
...
public: // find
    ...
    /// Slow: Find the first occurrence of the decoded UTF-8 text.
    /// Invalid UTF-8 in `text` is matched as the replacement character.
    [[nodiscard]] auto find(const U8StringView &text, CharCompareFn compareFn = {}) const noexcept -> unit::CpIndex;
    /// Slow: Find the first occurrence of the decoded UTF-8 text at or after the given character index.
    [[nodiscard]] auto find(const U8StringView &text, unit::CpIndex start, CharCompareFn compareFn = {}) const noexcept
        -> unit::CpIndex;
...
private:
    impl::U8StringViewStorage _storage; ///< The view storage.
};
