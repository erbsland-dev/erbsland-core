...
static_assert(std::is_same_v<decltype(std::declval<const el::stream::AnyStringBuilderStream &>().toString()), String>);
static_assert(std::is_same_v<decltype(std::declval<el::stream::AnyStringBuilderStream &>().takeString()), String>);
static_assert(std::
        is_same_v<decltype(std::declval<const el::stream::AnyStringBuilderStream &>().toStringEditor()), StringEditor>);

TESTED_TARGETS(AnyStringBuilderStream TextPrintContext PrintContextCommonBuilder PrintContextToBuilder)
class AnyStringBuilderStreamTest final : public el::UnitTest {
public:
    void testConstructionAndEncoding() {
        const auto u8Stream = el::stream::AnyStringBuilderStream::create(StringKind::U8);
        const auto u16Stream = el::stream::AnyStringBuilderStream::create(StringKind::U16);
        const auto u32Stream = el::stream::AnyStringBuilderStream::create(StringKind::U32);
    ...
    void testWriteMethodsAndClear() {
        ...
        REQUIRE_EQUAL(
            StringConverter{stream->toAnyString().toU8String()}.toStdString(), std::string{"Hello World\n\n"});
    ...
    void testPrintWritesThroughBuilderContext() {
        auto stream = el::stream::AnyStringBuilderStream::create(StringKind::U32);
    ...
    void testTakeMethodsResetButKeepKind() {
        ...
        stream->writeLine(StringEditor{std::string_view{"first"}});
        ...
        stream->writeLine(StringEditor{std::string_view{"second"}});
        ...
        stream->writeLine(StringEditor{std::string_view{"third"}});
        ...
        stream->writeLine(StringEditor{std::string_view{"fourth"}});
    ...
    void testFlushAndCloseState() {
        auto stream = el::stream::AnyStringBuilderStream::create(StringKind::U8);

        stream->writeLine(StringEditor{std::string_view{"open"}});
        stream->flush();
        stream->close();

        REQUIRE_FALSE(stream->isOpen());
        REQUIRE_THROWS(stream->writeLine(StringEditor{std::string_view{"closed"}}));
