...
void TlsClientConnection::handleHostResolved(const util::List<IpEndpoint> &endpoints) {
    ...
    if (!canContinue() || _state.load() != ConnectionState::Connecting) {
        return;
    }
    _hostResolved = true;
    try {
...
void TlsClientConnection::handleTransportConnected() {
    const auto lock = std::scoped_lock{_mutex};
    if (!canContinue() || _state.load() != ConnectionState::Connecting || _protocol == nullptr) {
        return;
    }
...
void TlsClientConnection::handleTransportWritable() {
    ...
    pumpTransportOutput();
    assessProtocolState();
    if (_applicationBlocked && _pendingTransportRecord == std::nullopt && _state.load() == ConnectionState::Active) {
