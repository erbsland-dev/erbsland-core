...
class BlockStringEditorTest final : public UNITTEST_SUBCLASS(TestHelper) {
public:
    void testUtf8StringIsSplitIntoTerminalCharacters() {
        const auto text = BlockStringEditor{"äöü"_el};
    ...
    void testUtf32StringIsSplitIntoTerminalCharacters() {
        const auto text = BlockStringEditor{U"äöü"_el};
        REQUIRE_EQUAL(text.length(), BlockCount{3});
        REQUIRE_EQUAL(text[BlockIndex{0}], U'ä');
        REQUIRE_EQUAL(text[BlockIndex{1}], U'ö');
        REQUIRE_EQUAL(text[BlockIndex{2}], U'ü');
    ...
    void testDisplayWidth() {
        const auto text = BlockStringEditor{"A中B"_el};
    ...
    void testSubstrReturnsRequestedWindowAndClampsBounds() {
        const auto text = BlockStringEditor{"AB界D"_el};
    ...
    void testIndexedAccessAndAtRespectCroppedBounds() {
        const auto text = BlockStringEditor{"012345"_el};
    ...
    void testMutableIndexedAccessIgnoresOutOfBounds() {
        auto text = BlockStringEditor{"ABC"_el}.slice(BlockRange{BlockIndex{1U}, BlockCount{2U}});
    ...
    void testTrimRemovesNonWhitespaceCharactersFromBothEndsOnly() {
        auto text = BlockStringEditor{"xyalpha yx"_el};
    ...
    void testTrimWorksOnCroppedStringsAndKeepsSharedCopiesIndependent() {
        const auto source = BlockStringEditor{"xx  alpha  yy"_el};
    ...
    void testNormalizeWorksOnCroppedStringsAndKeepsSharedCopiesIndependent() {
        const auto source = BlockStringEditor{"xx  alpha\t beta  yy"_el};
    ...
    void testReplaceCharacterCoversSingleMultiAndEmptyReplacement() {
        auto text = BlockStringEditor{"ABCDE"_el};
    ...
    void testReplaceStringCoversShorterLongerAndSameSizeReplacement() {
        auto text = BlockStringEditor{"ABCDE"_el};

        text.replace(BlockRange{BlockIndex{1}, BlockCount{2}}, BlockString{BlockStringEditor{"x"_el}});
        REQUIRE_EQUAL(render(text), std::string{"AxDE"});

        text.replace(BlockRange{BlockIndex{2}, BlockCount{1}}, BlockString{BlockStringEditor{"YZ"_el}});
        REQUIRE_EQUAL(render(text), std::string{"AxYZE"});

        text.replace(BlockRange{BlockIndex{1}, BlockCount{2}}, BlockString{BlockStringEditor{"12"_el}});
        REQUIRE_EQUAL(render(text), std::string{"A12ZE"});
    }

    void testReplaceStringSameSizeCopiesEveryChangedCharacter() {
        auto text = BlockStringEditor{"ABCDE"_el};

        text.replace(BlockRange{BlockIndex{1}, BlockCount{3}}, BlockString{BlockStringEditor{"xyz"_el}});

        REQUIRE_EQUAL(render(text), std::string{"AxyzE"});
    }

    void testReplaceHandlesOutOfBoundsAndCroppedStrings() {
        const auto source = BlockStringEditor{"0123456789"_el};
        ...
        cropped.replace(BlockRange{BlockIndex{1}, BlockCount{3}}, BlockString{BlockStringEditor{"XY"_el}});
    ...
    void testIndexOfCharacterSetCoversBoundsAndCroppedStrings() {
        const auto text = BlockStringEditor{"abc-def"_el};
        ...
        const auto cropped = BlockStringEditor{"xxabc-yy"_el}.slice(BlockRange{BlockIndex{2U}, BlockCount{4U}});
    ...
    void testIndexNotOfCharacterSetCoversBoundsAndCroppedStrings() {
        const auto text = BlockStringEditor{"aaabbb"_el};
    ...
    void testWrapIntoLinesUsesTerminalWidth() {
        const auto text = BlockStringEditor{"漢字テスト"_el};
    ...
    void testWrapIntoLinesUsesSingleParagraphSpacingByDefault() {
        const auto text = BlockStringEditor{"alpha beta\ngamma"_el};
    ...
    void testWrapIntoLinesPreservesExplicitEmptyParagraphs() {
        const auto lines = BlockStringEditor{"alpha\n\nbeta"_el}.wrapIntoLines(10);
    ...
    void testSplitLinesHandlesEmptyLeadingAndSingleNewlineStrings() {
        REQUIRE(BlockStringEditor{}.splitLines().empty());

        const auto leadingEmptyLine = BlockStringEditor{"\nalpha"_el}.splitLines();
        ...
        const auto singleNewline = BlockStringEditor{"\n"_el}.splitLines();
    ...
    void testWrapIntoLinesTrimsOuterSpacingAndKeepsInternalSpacingThatFits() {
        const auto lines = BlockStringEditor{"  alpha   beta  "_el}.wrapIntoLines(12);

        REQUIRE_EQUAL(renderLines(lines), std::vector<std::string>({"alpha   beta"}));
    }

    void testWrapIntoLinesSplitsOversizedWordsIntoStandaloneChunks() {
        const auto lines = BlockStringEditor{"supercalifragilistic test"_el}.wrapIntoLines(5);
    ...
    void testWrapIntoLinesSplitsWideWordsAtCharacterBoundaries() {
        const auto lines = BlockStringEditor{"漢字abcde"_el}.wrapIntoLines(5);
    ...
    void testWrapIntoLinesDoesNotDuplicateWordsBetweenTokens() {
        const auto lines =
            BlockStringEditor{"Text Gallery  |  alignment, wrapping, wide characters, and bitmap fonts"_el}
                .wrapIntoLines(200);
    ...
    void testTerminalLinesCountsWrappedAndExplicitLineBreaks() {
        REQUIRE_EQUAL(BlockStringEditor{}.terminalLines(4), 0);
        REQUIRE_EQUAL(BlockStringEditor{"AA BB"_el}.terminalLines(2), 3);
        REQUIRE_EQUAL(BlockStringEditor{"AA\nBB"_el}.terminalLines(2), 3);
    ...
    void testWrappedTextHeightUsesPlainOutputFallbackForInvalidParagraphSettings() {
        ...
        REQUIRE_EQUAL(BlockStringEditor{"AA BB"_el}.wrappedBlockTextHeight(blockCoordinate(2), options), 2);

        options.setOnError(ParagraphOnError::Empty);
        REQUIRE_EQUAL(BlockStringEditor{"AA BB"_el}.wrappedBlockTextHeight(blockCoordinate(2), options), 0);
    }

    void testCombiningCharactersStayInOneCell() {
        const auto text = BlockStringEditor{erbsland::text::StringEditor{bytes({0x65, 0xCC, 0x81})}};
        REQUIRE_EQUAL(text.length(), BlockCount{1});
        REQUIRE_EQUAL(text.displayWidth(), 1);
        REQUIRE_EQUAL(text[BlockIndex{0}].toString(), erbsland::text::StringEditor{bytes({0x65, 0xCC, 0x81})});
    }

    void testControlCodesAreFilteredExceptTabAndNewline() {
        const auto text = BlockStringEditor{"A\r\x01\t\nB"_el};
    ...
    void testAppendStyledAppendsUtf32TextWithoutChangingExistingCharacters() {
        auto text = BlockStringEditor{"A"_el};
        auto attributes = BlockAttributes{};
        attributes.setBold(true);
    ...
    void testStringAppendOperatorsReuseReadOnlyRanges() {
        const auto source = BlockStringEditor{"ABCDE"_el};
        const auto middle = BlockString{source}.slice(BlockRange{BlockIndex{1U}, BlockCount{3U}});

        auto appended = BlockStringEditor{"A"_el};
        appended += middle.slice(BlockRange{BlockIndex{0U}, BlockCount{2U}});

        const auto combined = BlockStringEditor{"A"_el} + middle.slice(BlockRange{BlockIndex{2U}, BlockCount{1U}});
    ...
    void testCroppedStringDetachesAfterReadOnlyRangeOperations() {
        const auto source = BlockStringEditor{"xxABC\nyy"_el};
    ...
    void testMutableIteratorsDetachBeforeWriting() {
        const auto source = BlockStringEditor{"ABCD"_el};
        auto copy = source;
    ...
    void testSharedCopiesSupportConcurrentReads() {
        const auto source = BlockStringEditor{"alpha beta gamma"_el};
    ...
    void testCopiedStringsCanMutateIndependentlyOnMultipleThreads() {
        const auto source = BlockStringEditor{"Base"_el};
        auto left = source;
        auto right = source;
    ...
    void testTrimmedSubstr() {
        const auto source = BlockStringEditor{"XXX  ABC  XXX"_el};
        auto substr = source.slice(BlockRange{BlockIndex{3U}, BlockCount{7U}});
        REQUIRE_EQUAL(substr, BlockStringEditor{"  ABC  "_el});
        auto trimmed = substr.trimmed();
        REQUIRE_EQUAL(trimmed, BlockStringEditor{"ABC"_el});
    }

    void testNormalizeSpecial() {
        auto source = BlockStringEditor{"\nXXX\n\tABC\n\tXXX\t"_el};
        source.normalize();
        REQUIRE_EQUAL(source, BlockStringEditor{"XXX ABC XXX"_el});
    }

    void testMovedFromStringKeepsEmptyInvariant() {
        auto source = BlockStringEditor{"alpha"_el};
        ...
        auto assigned = BlockStringEditor{"beta"_el};
...
private:
    ...
    [[nodiscard]] static auto renderLines(const BlockStringEditorLines &lines) -> std::vector<std::string> {
