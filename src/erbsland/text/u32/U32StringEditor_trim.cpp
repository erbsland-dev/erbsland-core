...
auto U32StringEditor::trim(const std::optional<CharSet> &characters, const std::optional<StringSide> side)
    -> U32StringEditor & {
...
auto U32StringEditor::trimmed(const std::optional<CharSet> &characters, const std::optional<StringSide> side) const
    -> U32StringEditor {
    const auto trimSide = impl::U32StringTrimTools::sideFrom(side);
    if (characters.has_value()) {
