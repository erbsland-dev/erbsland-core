...
template <impl::AnyFloatType T>
auto U8StringView::toFloat(T defaultValue, FloatParseOptions options) const noexcept -> T {
    if constexpr (std::same_as<T, float>) {
        return impl::parseFloatOrDefault(StringCharReader{*this}, defaultValue, options);
    } else {
        return impl::parseDoubleOrDefault(StringCharReader{*this}, defaultValue, options);
...
template <impl::AnyFloatType T>
auto U8StringView::toFloatOrThrow(FloatParseOptions options) const -> T {
    if constexpr (std::same_as<T, float>) {
        return impl::parseFloatOrThrow(StringCharReader{*this}, options);
    } else {
        return impl::parseDoubleOrThrow(StringCharReader{*this}, options);
...
template auto U8StringView::toFloat<float>(float defaultValue, FloatParseOptions options) const noexcept -> float;
template auto U8StringView::toFloat<double>(double defaultValue, FloatParseOptions options) const noexcept -> double;
template auto U8StringView::toFloatOrThrow<float>(FloatParseOptions options) const -> float;
template auto U8StringView::toFloatOrThrow<double>(FloatParseOptions options) const -> double;
