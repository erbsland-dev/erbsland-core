...
void printCharacterAtByByteIndex(const el::String &text, const el::ByteIndex byteIndex) {
    auto character = text.charAt(byteIndex);
    auto charText = el::StringEditor{};
...
/// `String` stores text as UTF-8. This means byte positions and character
...
/// frequently, consider `el::U32String` or `el::U32StringEditor`.
void indexing() {
    // Create an owning read-only string for the demo.
    const auto text = el::String{"🌲 Forêt 森"_el};
