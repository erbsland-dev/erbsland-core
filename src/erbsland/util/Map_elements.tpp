...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    ...
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::count() const noexcept -> Count {
...
auto Map<tKey, tValue, tCompare, tSelf>::countIf(Function function) const -> Count {
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    ...
    std::copyable<tValue>
template <typename Function>
auto Map<tKey, tValue, tCompare, tSelf>::countIfKey(Function function) const -> Count {
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    ...
    std::copyable<tValue>
template <typename Function>
auto Map<tKey, tValue, tCompare, tSelf>::countIfValue(Function function) const -> Count {
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    requires std::default_initializable<tKey> && std::default_initializable<tValue> && std::copyable<tKey> &&
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::get(const Key &key) const -> std::optional<Value> {
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    requires std::default_initializable<tKey> && std::default_initializable<tValue> && std::copyable<tKey> &&
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::get(const Key &key, const Value &defaultValue) const -> Value {
    const auto iterator = raw().find(key);
    if (iterator == raw().end()) {
        return defaultValue;
    }
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    requires std::default_initializable<tKey> && std::default_initializable<tValue> && std::copyable<tKey> &&
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::first() const -> Entry {
    const auto &data = raw();
    if (data.empty()) {
        return {};
    }
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    requires std::default_initializable<tKey> && std::default_initializable<tValue> && std::copyable<tKey> &&
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::last() const -> Entry {
    const auto &data = raw();
    if (data.empty()) {
        return {};
    }
    const auto iterator = data.rbegin();
    return {iterator->first, iterator->second};
}
...
template <typename tKey, typename tValue, typename tCompare, typename tSelf>
    requires std::default_initializable<tKey> && std::default_initializable<tValue> && std::copyable<tKey> &&
    std::copyable<tValue>
auto Map<tKey, tValue, tCompare, tSelf>::toList() const -> List<Entry> {
