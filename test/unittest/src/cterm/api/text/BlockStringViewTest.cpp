...
#include <erbsland/cterm/BlockStringView.hpp>
#include <erbsland/unittest/UnitTest.hpp>

#include <string>
#include <vector>

TESTED_TARGETS(BlockStringView)
class BlockStringViewTest final : public el::UnitTest {
public:
    void testStringImplicitlyConvertsToStringView() {
        ...
        const BlockStringView view = text;

        REQUIRE_EQUAL(view.length(), BlockCount{2U});
        REQUIRE_EQUAL(view[BlockIndex{0U}], U'A');
        REQUIRE_EQUAL(view[BlockIndex{1U}], U'B');
        REQUIRE_EQUAL(view[BlockIndex{0U}].color(), Color(fg::Red, bg::Black));
        REQUIRE_EQUAL(view[BlockIndex{1U}].color(), Color(fg::Green, bg::Black));
    }

    void testStringCanBeExplicitlyConstructedFromAView() {
        const auto source = BlockString{"AB界D"_el};
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{1U}, BlockCount{2U}});
        const auto copy = BlockString{view};
    ...
    void testSubstrAndTrimmedReturnViews() {
        const auto source = BlockString{"  alpha beta  "_el};
        const auto trimmed = BlockStringView{source}.trimmed(erbsland::text::CharSet{" "_el});
    ...
    void testIndexedAccessAndAtRespectCroppedBounds() {
        const auto source = BlockString{"012345"_el};
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{2U}, BlockCount{3U}});
    ...
    void testTrimmedUsesDefaultWhitespaceCharacters() {
        const auto source = BlockString{"xx \talpha\n yy"_el};
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{2U}, BlockCount{9U}});
    ...
    void testTrimmedRemovesNonWhitespaceCharactersFromCroppedView() {
        const auto source = BlockString{"00xyalpha yx11"_el};
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{2U}, BlockCount{10U}});
    ...
    void testIndexOfCharacterSetCoversBoundsAndCroppedViews() {
        const auto source = BlockString{"xxabc-yy"_el};
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{2U}, BlockCount{4U}});
    ...
    void testSplitWordsReturnsOnlyWordViews() {
        const auto words = BlockStringView{BlockString{"  alpha\tbeta\n\ngamma  "_el}}.splitWords();
    ...
    void testSplitLinesPreservesEmptyLinesWithoutTrailingEmptyLine() {
        const auto lines = BlockStringView{BlockString{"alpha\n\nbeta\n"_el}}.splitLines();
    ...
    void testCroppedViewsUseTheSameRangeAlgorithmsAsStrings() {
        const auto source = BlockString{U"xxA界 e\u0301\nB yy"_el};
        const auto stringRange = source.slice(BlockRange{BlockIndex{2U}, BlockCount{6U}});
        const auto view = BlockStringView{source}.slice(BlockRange{BlockIndex{2U}, BlockCount{6U}});
    ...
    void testEmptyViewRangesStayEmpty() {
        const auto view = BlockStringView{BlockString{"xxx"_el}}.trimmed(erbsland::text::CharSet{"x"_el});
...
private:
    [[nodiscard]] static auto render(const BlockStringView text) -> std::string {
    ...
    [[nodiscard]] static auto renderWords(const std::vector<BlockStringView> &words) -> std::vector<std::string> {
