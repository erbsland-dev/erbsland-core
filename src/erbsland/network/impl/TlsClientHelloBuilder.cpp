...
TlsClientHelloBuilder::TlsClientHelloBuilder(Host host, std::vector<mem::ByteBlock> alpnProtocols) :
    ...
    for (const auto &protocol : _alpnProtocols) {
        if (protocol.isEmpty() || protocol.length().toSizeT() > std::numeric_limits<uint8_t>::max()) {
            throw err::ParameterError{
                "Each ALPN protocol must contain between 1 and 255 bytes."_el, "alpnProtocols"_el};
        }
        encodedLength += 1U + protocol.length().toSizeT();
    }
...
auto TlsClientHelloBuilder::buildExtensions(const cryptology::KeyAgreementPublicKey &publicKey) const
    ...
    if (const auto name = _host.name(); name.has_value()) {
        ...
        const auto nameData = text::impl::UnsafeU8StringAccess{asciiName}.dataView().dataSpan();
    ...
    if (!_alpnProtocols.empty()) {
        ...
        for (const auto &protocol : _alpnProtocols) {
            protocolNames.writeVector8(protocol.span());
        }
