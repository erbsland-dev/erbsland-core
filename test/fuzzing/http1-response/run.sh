...
corpusDir="${buildDir}/corpus/http1-response"
...
cmake --build "${buildDir}" --target http1-response-fuzz
mkdir -p "${corpusDir}"
for source in "${scriptDir}"/corpus/*.hex; do xxd -r -p "${source}" "${corpusDir}/$(basename "${source}" .hex)"; done
exec "${buildDir}/test/fuzzing/http1-response/http1-response-fuzz" "${corpusDir}" "$@"
