...
public:
    ...
    static constexpr auto cMaximumBitmapSize = block::Size{4096, 4096};
...
public:
    ...
    explicit Bitmap(const block::Size size) :
...
public: // accessors
    ...
    [[nodiscard]] auto rect() const noexcept -> block::Rectangle {
        return block::Rectangle{block::Position{0, 0}, _size};
    }
    ...
    [[nodiscard]] auto pixel(block::Position pos) const noexcept -> bool;
    ...
    [[nodiscard]] auto pixelQuad(block::Position pos) const noexcept -> uint8_t;
    ...
    [[nodiscard]] auto pixelCardinal(block::Position pos) const noexcept -> uint8_t;
    ...
    [[nodiscard]] auto pixelRing(block::Position pos) const noexcept -> uint8_t;
    ...
    [[nodiscard]] auto boundingRect(bool value = true) const noexcept -> block::Rectangle;
...
public: // transformation
    ...
    [[nodiscard]] auto expanded(block::Margins margins, bool value) const noexcept -> Bitmap;
...
public: // drawing
    ...
    void draw(block::Position position, const std::vector<T> &data) noexcept;
    /// Draw another bitmap to the given position.
    /// @param position The top left corner of the bitmap to draw.
    /// @param bitmap The bitmap to draw.
    void draw(const block::Position position, const Bitmap &bitmap) noexcept {
        bitmap.size().forEach(
            [&](const block::Position delta) -> void { setPixel(position + delta, bitmap.pixel(delta)); });
    }
...
public: // conversion
    ...
    template <typename Fn>
        requires std::is_invocable_r_v<bool, Fn, block::Position>
    [[nodiscard]] static auto fromFunction(const block::Size size, Fn fn) noexcept -> Bitmap {
        Bitmap bitmap{size};
        bitmap.size().forEach([&](const block::Position pos) -> void { bitmap.setPixel(pos, fn(pos)); });
        return bitmap;
    }
...
protected:
    ...
    [[nodiscard]] auto pixelRef(const block::Position pos) -> Data::reference {
        if (!_size.contains(pos)) {
            throw err::OutOfRangeError{"block::Position out of bounds."};
        }
...
template <typename T>
    requires std::is_integral_v<T> && std::is_unsigned_v<T>
void Bitmap::draw(const block::Position position, const std::vector<T> &data) noexcept {
    for (std::size_t y = 0; y < data.size(); ++y) {
        std::bitset<sizeof(T) * 8> row(data[y]);
        for (auto x = block::Coordinate{0}; x < _size.width(); ++x) {
            setPixel(position + block::Position{x, block::Coordinate{y}}, row[x.toSizeT()]);
        }
    }
}
