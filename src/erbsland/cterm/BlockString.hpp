...
///
/// `BlockString` is implicitly shared. Copying a string shares its backing data, and a deep copy is only made when one
...
public:
    ...
    explicit BlockString(const BlockStringView &view);
...
public: // operators
    ...
    auto operator==(const BlockString &other) const noexcept -> bool;
    auto operator!=(const BlockString &other) const noexcept -> bool { return !operator==(other); }
    ...
    auto operator+=(const Block &character) noexcept -> BlockString & {
        append(character);
        return *this;
    }
    ...
    /// @return Reference to this string.
    auto operator+=(const BlockString &other) noexcept -> BlockString &;
    /// Append a read-only string view to this string.
    /// @param other The string view to append.
    /// @return Reference to this string.
    auto operator+=(const BlockStringView &other) noexcept -> BlockString &;
    /// Create a new string with one appended character.
    /// @param character The character to append.
    /// @return The concatenated string.
    auto operator+(const Block &character) const noexcept -> BlockString {
        auto copy = *this;
    ...
    /// @param other The string to append.
    /// @return The concatenated string.
    auto operator+(const BlockString &other) const noexcept -> BlockString;
    /// Create a new string with an appended read-only view.
    /// @param other The string view to append.
    /// @return The concatenated string.
    auto operator+(const BlockStringView &other) const noexcept -> BlockString;
...
public: // modifiers
    ...
    /// @overload
    void replace(BlockRange range, const BlockStringView &replacement) noexcept;
    ...
    void insertWithBaseStyle(BlockIndex pos, const BlockStringView &other, BlockStyle style) noexcept;
    ...
    void appendStyled(
        ...
        text::EncodingErrorMode encodingErrorMode = text::EncodingErrorMode::Replace);
    /// @overload
    void appendStyled(const text::U32StringView &text, BlockStyle style) noexcept;
    /// Append another terminal string with a base style.
    /// @param other The source text view.
    /// @param style The style used as base for inherited components in the appended range.
    void appendStyled(const BlockStringView &other, BlockStyle style) noexcept;
    ...
    /// Appended `Block`, `BlockString`, and `BlockStringView` values with inherited color components or inherited
...
public: // conversion
    ...
    [[nodiscard]] static auto fromLines(
        std::initializer_list<text::StringView> lines, Color color = {}, BlockAttributes attributes = {}) noexcept
        -> BlockString;
    /// @overload
    [[nodiscard]] static auto fromLines(std::initializer_list<text::StringView> lines, BlockStyle style) noexcept
        -> BlockString;
    /// @overload
    [[nodiscard]] static auto fromLines(
        std::initializer_list<text::U32StringView> lines, Color color = {}, BlockAttributes attributes = {}) noexcept
        -> BlockString;
    /// @overload
    [[nodiscard]] static auto fromLines(std::initializer_list<text::U32StringView> lines, BlockStyle style) noexcept
        -> BlockString;

private:
    friend class BlockStringView;
    friend class impl::BlockPrintContextToBlockString;
    friend class impl::BlockStringBuilder;

    explicit BlockString(impl::BlockStringDataPtr data, BlockRange range) noexcept;
    explicit BlockString(Storage chars) noexcept;
    [[nodiscard]] static auto fromStorageWithDisplayWidth(Storage chars, int displayWidth) noexcept -> BlockString;
    ...
    [[nodiscard]] static auto splitCharacters(
        const text::U32StringView &str, Color color = {}, BlockAttributes attributes = {}) -> Storage;
    [[nodiscard]] auto createPrintContext() noexcept -> BlockPrintContextPtr;
    void appendView(const BlockStringView &view, BlockStyle style) noexcept;
    void appendViewWithBaseStyle(const BlockStringView &view, BlockStyle style) noexcept;
    void detach();
