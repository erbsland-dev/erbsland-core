...
corpusDir="${buildDir}/corpus/tls-client-protocol"
...
cmake --build "${buildDir}" --target tls-client-protocol-fuzz

mkdir -p "${corpusDir}"
for seed in "${scriptDir}"/corpus/*.hex; do
    xxd -r -p "${seed}" "${corpusDir}/$(basename "${seed}" .hex)"
done
exec "${buildDir}/test/fuzzing/tls-client-protocol/tls-client-protocol-fuzz" "${corpusDir}" "$@"
