...
using el::text::U32StringView;

TESTED_TARGETS(U32StringLiteral operator_el operator_elv operator_els)
...
public:
    void testConstexprUtf32LiteralConstructor() {
        ...
        const auto view = U32StringView{literal};
        const auto text = U32String{literal};
    ...
    void testUtf32LiteralOperatorCreatesLiteralViewAndString() {
        ...
        const auto literalView = U32StringView{literal};
        const auto literalText = U32String{literal};
        const auto view = U"Hello"_elv;
        const auto text = U"Hello"_els;

        static_assert(std::is_same_v<decltype(literal), const U32StringLiteral>);
        static_assert(std::is_same_v<decltype(view), const U32StringView>);
        static_assert(std::is_same_v<decltype(text), const U32String>);
    ...
    void testViewsAreNotConstructibleFromPointersOrStringViews() {
        static_assert(!std::is_constructible_v<U32StringView, const char32_t *>);
        static_assert(!std::is_constructible_v<U32StringView, std::u32string_view>);
