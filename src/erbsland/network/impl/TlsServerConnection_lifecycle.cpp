...
void TlsServerConnection::ensureIdleTimer() {
    if (_idleTimerPending || _state.load() != TlsServerConnectionState::Active) {
        return;
    }
...
void TlsServerConnection::handleHandshakeTimeout(const std::uint64_t generation, const time::TimePoint deadline) {
    ...
    if (generation != _handshakeTimerGeneration || _state.load() != TlsServerConnectionState::Handshaking ||
...
void TlsServerConnection::handleIdleTimeout(const std::uint64_t generation) {
    ...
    if (_state.load() != TlsServerConnectionState::Active) {
        return;
    }
...
void TlsServerConnection::handleCloseTimeout(const std::uint64_t generation, const time::TimePoint deadline) {
    const auto lock = std::scoped_lock{_mutex};
    if (generation != _closeTimerGeneration || _state.load() != TlsServerConnectionState::Closing ||
        time::TimePoint::now() < deadline) {
        return;
    }
    finishFailed(
...
void TlsServerConnection::enterClosing(const TlsServerConnectionCloseOrigin origin) {
    if (_closeOrigin.has_value()) {
        return;
    }
    _closeOrigin = origin;
    _state.store(TlsServerConnectionState::Closing);
...
auto TlsServerConnection::protocolErrorContext() const -> NetworkErrorContext {
    ...
    const auto phase = _state.load() == TlsServerConnectionState::Closing ? NetworkErrorPhase::Closing
        : _state.load() == TlsServerConnectionState::Active               ? NetworkErrorPhase::Active
...
auto TlsServerConnection::enrichTransportError(NetworkErrorContext context) const -> NetworkErrorContext {
    ...
    switch (_state.load()) {
    case TlsServerConnectionState::Inactive:
        phase = NetworkErrorPhase::Configuration;
        break;
    case TlsServerConnectionState::Accepting:
        phase = NetworkErrorPhase::Accepting;
        break;
    case TlsServerConnectionState::Handshaking:
        phase = NetworkErrorPhase::Handshaking;
        break;
    case TlsServerConnectionState::Active:
        phase = NetworkErrorPhase::Active;
        break;
    case TlsServerConnectionState::Closing:
    case TlsServerConnectionState::Closed:
        phase = NetworkErrorPhase::Closing;
        break;
    case TlsServerConnectionState::Failed:
        phase = context.phase();
        break;
    }
    context.setPhase(phase);
...
void TlsServerConnection::finishFailed(NetworkErrorContext context) {
    if (_finalized.load() || _state.load() == TlsServerConnectionState::Failed) {
        return;
    }
...
void TlsServerConnection::finishClosed() {
    ...
    _state.store(TlsServerConnectionState::Closed);
    ++_closeTimerGeneration;
    const auto context = TlsServerConnectionCloseContext{_closeOrigin.value_or(TlsServerConnectionCloseOrigin::Remote)};
    auto firstException = std::exception_ptr{};
    try {
