...
class UtilityApp:
    ...
    def config_file_path(self) -> Path:
        ...
        config_directory = "_conf" if self.private_config else "conf"
        return module_path.parent.parent / config_directory / f"{module_path.stem}.elcl"
    ...
    def parse_command_line(self, argv: Sequence[str] | None = None) -> argparse.Namespace:
        ...
        parser.add_argument(
            "--private-config",
            action="store_true",
            help="Read this utility's configuration from utilities/_conf instead of utilities/conf.",
        )
        self.add_command_line_args(parser)
        args = parser.parse_args(argv)
        self.verbose = args.verbose
        self.private_config = args.private_config
