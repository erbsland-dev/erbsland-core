...
void TcpConnection::close() {
    ...
    if (current == NetworkSourceState::Closed || current == NetworkSourceState::Failed ||
        current == NetworkSourceState::Closing) {
        return;
    }
    _started.store(true);
    _closeOrigin = TcpConnectionCloseOrigin::Local;
    _state.store(NetworkSourceState::Closing);
    if (_lookup != nullptr) {
        _lookup->cancel();
        _lookup.reset();
    }
    updateReceiving();
    scheduleDataDelivery();
    if (current == NetworkSourceState::Starting) {
        _sendQueue.clear();
        _sendQueueSize = {};
        _receiveQueue.clear();
        _receiveQueueSize = {};
        finishClosed(TcpConnectionCloseOrigin::Local);
        return;
    }
    flushOutput();
    finishCloseIfReady();
...
void TcpConnection::postData(const std::uint64_t generation, mem::ByteBlock data) {
    const auto lifecycleLock = std::scoped_lock{_lifecycleMutex};
    if (!isCurrent(generation) || _state.load() != NetworkSourceState::Active || data.isEmpty()) {
        return;
    }
...
void TcpConnection::deliverData(const std::uint64_t generation) {
    ...
    const auto current = _state.load();
    if (!isCurrent(generation) || (current != NetworkSourceState::Active && current != NetworkSourceState::Closing) ||
        _receiveQueue.empty() || (_receivePaused && current == NetworkSourceState::Active)) {
        return;
    }
...
void TcpConnection::deliverSendCompleted(const std::uint64_t generation) {
    ...
    if (!isCurrent(generation) || !_sendPending ||
        (current != NetworkSourceState::Active && current != NetworkSourceState::Closing)) {
        return;
    }
    _sendPending = false;
    finishAcceptedBlock();
...
void TcpConnection::deliverRemoteClosed(const std::uint64_t generation) {
    ...
    _closeOrigin = TcpConnectionCloseOrigin::Remote;
    _state.store(NetworkSourceState::Closing);
    updateReceiving();
    scheduleDataDelivery();
    flushOutput();
...
void TcpConnection::flushOutput() {
    const auto current = _state.load();
    if ((current != NetworkSourceState::Active && current != NetworkSourceState::Closing) || _sendPending) {
        return;
    }
...
void TcpConnection::updateReceiving() {
    ...
    if (_state.load() == NetworkSourceState::Active && !_receivePaused && _receiveQueueSize < _bufferLimits.receive()) {
...
void TcpConnection::scheduleDataDelivery() {
    const auto current = _state.load();
    if (_receiveDeliveryPosted || _receiveQueue.empty() || (_receivePaused && current == NetworkSourceState::Active) ||
        (current != NetworkSourceState::Active && current != NetworkSourceState::Closing)) {
        return;
    }
...
void TcpConnection::finishCloseIfReady() {
    if (_state.load() != NetworkSourceState::Closing || !_sendQueue.empty() || _sendPending || !_receiveQueue.empty() ||
...
void TcpConnection::finishClosed(const TcpConnectionCloseOrigin origin) {
    const auto lifecycleLock = std::scoped_lock{_lifecycleMutex};
    if (_state.exchange(NetworkSourceState::Closed) == NetworkSourceState::Closed) {
        return;
    }
    cleanupDevice(false);
    postClosedAndFinal(_generation.load(), TcpConnectionCloseContext{origin});
}
...
void TcpConnection::postClosedAndFinal(const std::uint64_t generation, const TcpConnectionCloseContext context) {
    const auto weakSelf = std::weak_ptr<TcpConnection>{std::static_pointer_cast<TcpConnection>(shared_from_this())};
