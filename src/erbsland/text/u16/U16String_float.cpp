...
auto U16String::fromFloat(const double value, const FloatFormat format) -> U16String {
    ...
    return StringConverter{text}.toU16String();
}
...
template <impl::AnyFloatType T>
auto U16StringView::toFloat(T defaultValue, FloatParseOptions options) const noexcept -> T {
    if constexpr (std::same_as<T, float>) {
        return impl::parseFloatOrDefault(StringCharReader{*this}, defaultValue, options);
    } else {
        return impl::parseDoubleOrDefault(StringCharReader{*this}, defaultValue, options);
...
template <impl::AnyFloatType T>
auto U16StringView::toFloatOrThrow(FloatParseOptions options) const -> T {
    if constexpr (std::same_as<T, float>) {
        return impl::parseFloatOrThrow(StringCharReader{*this}, options);
    } else {
        return impl::parseDoubleOrThrow(StringCharReader{*this}, options);
...
template auto U16StringView::toFloat<float>(float defaultValue, FloatParseOptions options) const noexcept -> float;
template auto U16StringView::toFloat<double>(double defaultValue, FloatParseOptions options) const noexcept -> double;
template auto U16StringView::toFloatOrThrow<float>(FloatParseOptions options) const -> float;
template auto U16StringView::toFloatOrThrow<double>(FloatParseOptions options) const -> double;
