...
public:
    ...
    void testAppendToAndSize_CaptureGroups() {
        ...
        groups.emplace_back(1, CaptureRange{0, 2}, StringView{});
        groups.emplace_back(2, CaptureRange{2, 4}, StringView{});
        const auto match = std::make_shared<MockStringMatch>(ConstRegExPtr{}, std::move(groups), text);
        REQUIRE(match != nullptr);

        Replacement replacement;
        replacement.addStaticText(String{"<"_el});
        ...
        REQUIRE_EQUAL(out, String{"<cd>-ab-abcd"_el});
    }
};
