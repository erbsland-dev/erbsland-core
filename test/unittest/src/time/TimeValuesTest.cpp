...
public:
    void testMidnightValues() {
        ...
        for (auto lineNumber = std::size_t{1}; lineNumber <= lines.size(); ++lineNumber) {
            ...
            runWithContext(
                ...
                [&]() -> std::string {
                    return std::format("time midnight line={} ns={}", lineNumber, nanoseconds.toRawValue());
                });
        }
    }
    ...
    void testComparison() {
        ...
        requireComparisonRows(values, "Time");
    }

    void testStringConversion() {
        const auto midnightText = Time{}.toString();
        REQUIRE_EQUAL(midnightText, "00:00:00"_el);
        const auto fractionalText = Time{Hour{7}, Minute{4}, Second{3}, Nanoseconds{120'340'000}}.toString();
        REQUIRE_EQUAL(fractionalText, "07:04:03.12034"_el);
    }
};
