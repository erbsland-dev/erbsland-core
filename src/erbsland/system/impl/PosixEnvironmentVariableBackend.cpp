...
auto PosixEnvironmentVariableBackend::get(const text::String &name) const -> std::optional<text::String> {
    const auto nameAccess = text::impl::UnsafeU8StringAccess{name};
    const auto *value = std::getenv(nameAccess.data());
    if (value == nullptr) {
        return std::nullopt;
    }
...
void PosixEnvironmentVariableBackend::set(const text::String &name, const text::String &value) {
    const auto nameAccess = text::impl::UnsafeU8StringAccess{name};
    const auto valueAccess = text::impl::UnsafeU8StringAccess{value};
    const auto *valueData = value.isEmpty() ? "" : valueAccess.data();
    if (::setenv(nameAccess.data(), valueData, 1) != 0) {
